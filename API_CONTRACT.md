...
- **Default `1.0`** (trust Garmin until a real race says otherwise). Learned from actual results: on each completed race `observed = actual_time_s / garmin_predicted_time_s`, then EWMA `k ← (1-α)·k + α·observed`, **α = 0.35**. **Clamp `k ∈ [0.85, 1.25]`** (both directions — a runner may beat or miss Garmin; one blow-up/downhill-PR can't distort it). user 2 racing 2:28 against a Garmin 2:12 prediction pushes `k → ~1.13`, so her future 2:12 → ~2:28.
...
All backend + frontend landed; 278 backend tests pass, `tsc` clean. Live validation on both accounts (uncalibrated, `n_samples=0`): William 2:01:31 (range 1:52–2:11, his 1:52 goal at the fast end), user 2 2:12:30 (range 2:02–2:23) — both `source:"garmin"`, low confidence, matching Garmin exactly until a real race tunes `k`.
